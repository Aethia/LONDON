...
public class Partie implements Serializable{
	...
	public void calculPointsVictoire(){
		for(int i=0; i < nbJoueurs; i++){
			...
			int nb_emprunt = 0;
			//remboursement de l'emprunt (Â£15 pour Â£10 empruntÃ©es)
			if(listeJoueurs.get(i).getMontantEmprunts() > 0){
				...
				//nb_packet_argent est le nombre de packet de Â£15 que le joueur possÃ¨de
			...
			//-7 points de victoire par emprunt non remboursÃ©
	...
	public void calculGagnant(){
		...
		//parcours points de pauvretÃ© pour savoir qui a le moins, on supprime ce nombre pour tout le monde ce nombre
		int min = 999;
		for(int i = 0; i < nbJoueurs; i++){
		...
		for(int i = 0; i < nbJoueurs; i++){
			...
			//PenalitÃ© des points de pauvretÃ©
	...
	public void sauvegarder() throws IOException{
		...
		System.out.println("Objet sÃ©rialisÃ©");
		s.close();
	}
	...
	public void chargerPartie() throws IOException, ClassNotFoundException{
		...
		System.out.println("Partie chargÃ©");
		s.close();
